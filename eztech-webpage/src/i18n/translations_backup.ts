export const translations = {
  en: {
    nav: {
      home: 'Home',
      about: 'About Us',
      services: 'Services',
      products: 'Products',
      blog: 'Blog',
      contact: 'Contact Us',
      getQuote: 'Get Quote'
    },
    hero: {
      title: 'Revolutionizing Businesses with Cutting-Edge Technology',
      subtitle: 'Empowering digital transformation with AI, cloud solutions, and machine learning',
      exploreServices: 'Explore Services',
      contactUs: 'Contact Us'
    },
    about:{
      title:'About Us',
      subtitle1:'At EZ Tech, we make innovation easy. We empower businesses to work smarter, adapt faster, make data-driven decisions, and achieve more by harnessing the power of cutting-edge technology. With over 15 years of combined experience at leading AI and consulting firms worldwide, our expert team helps businesses thrive by leveraging tomorrow’s technologies, tailored to your unique goals. Our expertise spans across seamless cloud migrations, actionable data analytics, and custom AI and machine learning solutions — ensuring your business stays ahead in a competitive landscape.',
      subtitle2:"Our flagship products are crafted to help businesses harness the latest AI advancements. From EZChat, designed to deliver intelligent insights for complex problem-solving, to EZ Dash, offering powerful analytics, and EZ Note, streamlining workflow management, we enable businesses to visualize insights and boost productivity. At EZ Tech, technology isn't just a tool — it's the key to unlocking your potential.",
      highlightedSentence:"At EZ Tech, technology isn't just a tool — it's the key to unlocking your potential.",
      minititle:"Let's transform the future, together.",
      box1_title:'Simplify Innovation',
      box1_subtitle:'Making complex technology accessible and effective',
      box2_title:'Smart Solutions',
      box2_subtitle:'Empowering businesses with intelligent AI technology',
      box3_title:'AI Excellence',
      box3_subtitle:'Cutting-edge AI solutions for real-world challenges',
      box4_title:'Future Ready',
      box4_subtitle:"Preparing businesses leveraging AI for tomorrow's challenges",
    },
    services:{
      title:'Our Services',
      subtitle:'Comprehensive technology solutions to transform your business',
      items:{
        digital_transformation:{
          title:'Digital Transformation',
          subtitle:'We guide businesses through every step of their digital journey, from process optimization to implementing innovative technologies that reshape their operations.',
        },
        cloud_migration:{
          title:'Cloud Migration',
          subtitle:'We streamline cloud migration strategies to ensure secure, scalable, and efficient transitions, helping businesses embrace the power of the cloud with ease.',
        },
        ml_ai:{
          title:'Machine Learning and AI Solutions',
          subtitle:'We build machine learning models for forecasting, analytics, and customer insights while delivering AI solutions to automate processes and enhance experiences in real time.',
        },
        data_analytics:{
          title:'Web Scraping & Data Analytics',
          subtitle:'Our data experts collect data from websites and transform them into actionable insights that drive better decision-making and improve operational efficiency.',
        },
        dashboarding:{
          title:'Dashboarding',
          subtitle:'We create intuitive, interactive dashboards that visualize critical business metrics, providing businesses with a clear and concise view of their performance.',
        },
        consulting:{
          title:'Consulting',
          subtitle:'Our experienced consultants work closely with clients to provide tailored strategies, technical expertise, and industry insights that align with their business objectives.',
        }
      }
    },
    products: {
      title: 'Our Products',
      subtitle: 'Innovative solutions designed for modern businesses',
      learnMore: 'Learn More',
      learnNote:'Coming Soon',
      items: {
        EZChat: {
          title: 'EZ Chat',
          description: 'AI Assistant chatbot that revolutionizes customer support and internal communications with advanced natural language processing.',
          features: ['24/7 Customer Support', 'Multi-language Support', 'Custom Training']
        },
        'EZDash': {
          title: 'EZ Dash',
          description: 'Advanced dashboarding and analytics platform that transforms your data into actionable insights with real-time visualization.',
          features: ['Real-time Analytics', 'Custom Reports', 'Data Integration']
        },
        'EZNote': {
          title: 'EZ Note',
          description: 'Streamline collaboration with our intuitive note-taking and sharing platform for internal and external teams.',
          features: ['Real-time Collaboration', 'Version Control', 'Smart Organization']
        }
      },
      commingSoon:{
        title:'Coming Soon!',
        subtitle:"We're working hard to bring something amazing to you.",
        subtitle2:'Stay tuned! connect with us for updates.',
        days:'days',
        hours:'hours',
        minutes:'minutes',
        second:'second'

      }
    },
    contact: {
      title: 'Get in Touch',
      subtitle: 'Have a question or want to learn more? We\'d love to hear from you.',
      form: {
        name: 'Full Name',
        email: 'Email Address',
        phone: 'Phone Number',
        message: 'Your Message',
        submit: 'Send Message',
        sending: 'Sending',
        success: 'Message sent successfully!',
        error: 'There was an error sending your message. Please try again.'
      }
    },
    footer:{
      title:'EZ Tech',
      subtitle:'Innovating for a better future.',
      company:["About Us",  "Contact Us"],
      company_title:'Company',
      contact:'Connect With Us',
      copyright:'Copyright 2024. All Rights Reserved.',

    },
  },
  ru: {
    nav: {
      home: 'Главная',
      about: 'О Нас',
      services: 'Услуги',
      products: 'Продукты',
      blog: 'Блог',
      contact: 'Контакты',
      getQuote: 'Получить Цену'
    },
    hero: {
      title: 'Революционные Технологии для Бизнеса',
      subtitle: 'Цифровая трансформация с помощью ИИ, облачных решений и машинного обучения',
      exploreServices: 'Изучить Услуги',
      contactUs: 'Связаться с Нами'
    },
    about: {
      title: 'О Нас',
      subtitle1: 'В EZ Tech мы упрощаем инновации. От цифровой трансформации до решений, основанных на ИИ, мы помогаем бизнесу работать умнее, адаптироваться быстрее, принимать решения на основе данных и достигать большего. Наша экспертная команда предоставляет бесшовные облачные миграции, действенную аналитическую обработку данных и индивидуальные модели машинного обучения, адаптированные к вашим целям.',
      subtitle2: 'С такими инновационными продуктами, как EZChat для интеллектуальной коммуникации, EZ Dash для аналитики и EZ Note для эффективного управления рабочими процессами, мы помогаем бизнесу визуализировать данные и повышать продуктивность. В EZ Tech технологии — это не просто инструмент, это ключ к раскрытию вашего потенциала.',
      minititle: 'Давайте преобразим будущее вместе.',
      box1_title: 'Упростите Инновации',
      box1_subtitle: 'Делаем сложные технологии доступными и эффективными',
      box2_title: 'Умные Решения',
      box2_subtitle: 'Помогаем бизнесу с помощью интеллектуальных технологий',
      box3_title: 'Искусственный Интеллект на Высшем Уровне',
      box3_subtitle: 'Передовые ИИ-решения для реальных задач',
      box4_title: 'Готовность к Будущему',
      box4_subtitle: 'Подготовка бизнеса к вызовам завтрашнего дня'

    },
    services: {
      title: 'Наши Услуги',
      subtitle: 'Комплексные технологические решения для трансформации вашего бизнеса',
      items: {
        digital_transformation: {
          title: 'Цифровая Трансформация',
          subtitle: 'Мы сопровождаем бизнес на каждом этапе цифрового пути: от оптимизации процессов до внедрения инновационных технологий, которые меняют их работу.',
        },
        cloud_migration: {
          title: 'Миграция в Облако',
          subtitle: 'Мы упрощаем стратегии миграции в облако, обеспечивая безопасный, масштабируемый и эффективный переход, помогая бизнесу легко освоить возможности облачных технологий.',
        },
        ml_ai: {
          title: 'Машинное Обучение и Решения на Базе ИИ',
          subtitle: 'Мы создаем модели машинного обучения для прогнозирования, аналитики и понимания клиентов, а также предлагаем ИИ-решения для автоматизации процессов и улучшения взаимодействия в реальном времени.',
        },
        data_analytics: {
          title: 'Веб-скрапинг и Аналитика Данных',
	  subtitle: 'Наши эксперты по данным собирают информацию с веб-сайтов и преобразуют её в полезные аналитические данные, которые способствуют принятию более обоснованных решений и повышению операционной эффективности.'
        },
        dashboarding: {
          title: 'Создание Дашбордов',
          subtitle: 'Мы создаем интуитивно понятные, интерактивные дашборды, которые визуализируют ключевые бизнес-метрики, предоставляя компаниям ясное и краткое представление об их эффективности.',
        },
        consulting: {
          title: 'Консалтинг',
          subtitle: 'Наши опытные консультанты тесно работают с клиентами, предлагая индивидуальные стратегии, техническую экспертизу и отраслевые инсайты, которые соответствуют их бизнес-целям.',
        },
      },
    },
    
    
    products: {
      title: 'Наши Продукты',
      subtitle: 'Инновационные решения для современного бизнеса',
      learnMore: 'Подробнее',
      learnNote: 'Скоро будет',
      items: {
        EZChat: {
          title: 'EZ Chat',
          description: 'ИИ-чатбот, который революционизирует клиентскую поддержку и внутренние коммуникации с помощью продвинутой обработки естественного языка.',
          features: ['Поддержка 24/7', 'Многоязычная Поддержка', 'Индивидуальное Обучение']
        },
        'EZDash': {
          title: 'EZ Dash',
          description: 'Продвинутая платформа для дашбордов и аналитики, которая превращает ваши данные в полезные инсайты с визуализацией в реальном времени.',
          features: ['Аналитика в Реальном Времени', 'Настраиваемые Отчеты', 'Интеграция Данных']
        },
        'EZNote': {
          title: 'EZ Note',
          description: 'Оптимизируйте совместную работу с помощью нашей интуитивной платформы для заметок и обмена информацией для внутренних и внешних команд.',
          features: ['Совместная Работа в Реальном Времени', 'Контроль Версий', 'Умная Организация']
        }
      },
      commingSoon:{
        title:'Скоро будет доступно!',
        subtitle:'Мы усердно работаем, чтобы подготовить для вас что-то невероятное.',
        subtitle2:'Оставайтесь с нами! Свяжитесь с нами, чтобы получать обновления.'
      }
      
    },
    contact: {
      title: 'Свяжитесь с Нами',
      subtitle: 'Есть вопросы или хотите узнать больше? Мы будем рады вам помочь.',
      form: {
        name: 'Полное Имя',
        email: 'Электронная Почта',
        phone: 'Номер Телефона',
        message: 'Ваше Сообщение',
        submit: 'Отправить',
        sending: 'Отправка',
        success: 'Сообщение успешно отправлено!',
        error: 'Произошла ошибка при отправке сообщения. Пожалуйста, попробуйте снова.'
      }
    },
    footer: {
      title: 'EZ Tech',
      subtitle: 'Инновации ради лучшего будущего.',
      company: ["О нас", "Свяжитесь с нами"],
      company_title: 'Компания',
      contact: 'Свяжитесь с нами',
      copyright: 'Авторское право 2024. Все права защищены.',
    }
    
    
  }
};
